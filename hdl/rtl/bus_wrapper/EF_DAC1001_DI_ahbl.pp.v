...
module EF_DAC1001_DI_ahbl (
	...
	wire		_clk_	= HCLK;
	wire		_rst_	= ~HRESETn;
	wire		wr	= (ahbl_we & (last_HADDR[15:0]==DATA_REG_ADDR));
	...
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) CTRL_REG <= 0; else if(ahbl_we & (last_HADDR[15:0]==CTRL_REG_ADDR)) CTRL_REG <= HWDATA[32-1:0];
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) FIFOT_REG <= 0; else if(ahbl_we & (last_HADDR[15:0]==FIFOT_REG_ADDR)) FIFOT_REG <= HWDATA[5-1:0];
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) SAMPCTRL_REG <= 0; else if(ahbl_we & (last_HADDR[15:0]==SAMPCTRL_REG_ADDR)) SAMPCTRL_REG <= HWDATA[32-1:0];
