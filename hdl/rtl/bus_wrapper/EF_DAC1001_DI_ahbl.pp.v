...
module EF_DAC1001_DI_ahbl (
	...
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) CTRL_REG <= 0; else if(ahbl_we & (last_HADDR==CTRL_REG_ADDR)) CTRL_REG <= HWDATA;
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) FIFOT_REG <= 0; else if(ahbl_we & (last_HADDR==FIFOT_REG_ADDR)) FIFOT_REG <= HWDATA;
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) SAMPCTRL_REG <= 0; else if(ahbl_we & (last_HADDR==SAMPCTRL_REG_ADDR)) SAMPCTRL_REG <= HWDATA;
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) IM_REG <= 0; else if(ahbl_we & (last_HADDR==IM_REG_ADDR)) IM_REG <= HWDATA;

	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) ICR_REG <= 2'b0; else if(ahbl_we & (last_HADDR==ICR_REG_ADDR)) ICR_REG <= HWDATA; else ICR_REG <= 2'd0;
